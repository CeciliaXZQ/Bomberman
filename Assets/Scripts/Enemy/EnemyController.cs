...
public class EnemyController : MonoBehaviour, IDamagable, IKillable
{
    ...
    private void CheckAvailableDirection(Vector3 direction)
    {
        ...
        if (obj == null || obj.GetComponent<EnemyController>() != null || obj.GetComponent<FloorBlocks>() != null)
        {
    ...
    void LookAT2D(Vector2 startPos, Vector2 endPos)
    {
        ...
        }else if(endPos.x > startPos.x && enemySprite.rotation.y != 180)
        {
            enemySprite.rotation = Quaternion.Euler(new Vector3(0, 180, 0));
        }
    }
}
